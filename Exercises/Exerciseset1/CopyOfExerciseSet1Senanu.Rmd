...
1. The cumulative returns of the APPLE index during this period is `r round(100*sum(StockReturns[,2]),1)`%.
2. The average daily returns of the APPLE index during this period is `r round(100*mean(StockReturns[,2]),3)`%;
3. The standard deviation of the daily returns of the APPLE index during this period is `r round(100*sd(StockReturns[,2]),3)`%;

Here are returns of the APPLE in this period (note the use of the helper function pnl_plot - defined in file helpersSet1.R):
...
**Your Answers here:**
<br>Question 1: We specified it in line 8 of the code in dataSet1.R 
<br>Question 2:
<br>The cumulative returns of the APPLE index during this period is `r round(100*sum(StockReturns[,2]),1)`%.
<br>The average daily returns of the APPLE index during this period is `r round(100*mean(StockReturns[,2]),3)`%;
<br>The standard deviation of the daily returns of APPLE index during this period is `r round(100*sd(StockReturns[,2]),3)`%;
<br>Queston 3:
<br>1. The cumulative returns of the YAHOO index during this period is `r round(100*sum(StockReturns[,3]),1)`%.
<br>2. The average daily returns of the YAHOO index during this period is `r round(100*mean(StockReturns[,3]),3)`%;
<br>3. The standard deviation of the daily returns of the YAHOO index during this period is `r round(100*sd(StockReturns[,3]),3)`%
...
**Your Answers here:**
<br> ncol(transposedData) nrow(transposedData)


```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE,results='asis',fig.align='center', fig=TRUE}
portfolio2 = apply(stockreturns2,1,mean)
names(portfolio2) <- rownames(stockreturns2)
pnl_plot(portfolio2)
```
<br>
<br>
...
**Your Answers here:**
<br> Question 1: Line 9 - startDate = "2005-01-01". We rerun by source the dataSet1.R again
...
**Your Answers here:**
<br> Question 1: Returns a value of 20. The command run a logical test to compare both the variable myData against StockReturns for the first 20 rows and and all columns.

It returns "20" because the logical test is True and since True is equal to the numeric variable 1, it sums up the 20 rows of True.
